...
#pragma warning disable CA1712 // Do not prefix enum values with type name
    public class FolderBrowser : NativeWindow
    {
        private class ButtonHooker : NativeWindow
        {
        ...
        public DialogResult ShowDialog(IWin32Window owner)
        {
            ...
            try
            {
                ...
                {
                    dialog.SetOptions(FOS.FOS_FORCEFILESYSTEM);
                }

                if (!string.IsNullOrEmpty(Title))
                    dialog.SetTitle(Title);

                ButtonHooker hook = new ButtonHooker();
                hook.AssignHandle(NativeMethods.GetActiveWindow());
